/* Main app JS - cleaned and modularized (corrigido) */
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* safe localStorage parse (evita crash se JSON inválido) */
  function loadJSON(key, fallback = []) {
    try {
      ...
      console.warn(`Erro ao parsear ${key} do localStorage:`, err);
      return fallback;
    }
  }
  ...
  let presentes = loadJSON("presentes", []);
  let historico = loadJSON("historico", []);
  ...
  function renderQrForId(id) {
    ...
    el.innerHTML = "";
    try {
      // qrcodejs: cria <img> ou <canvas>, depende do browser
      new QRCode(el, { text: id, width: 160, height: 160, correctLevel: QRCode.CorrectLevel.H });
    } catch (e) {
  ...
  document.getElementById("registrarCadastro").onclick = () => {
    ...
    // limpar campos — agora trata checkboxes corretamente
    document.querySelectorAll("#cadastro input:not([readonly]), #cadastro select, #cadastro textarea").forEach(i => {
      if (i.type === "checkbox") {
        i.checked = false;
      } else {
        i.value = "";
  ...
  document.getElementById("exportCsv").onclick = () => {
    ...
    // permite tempo para o navegador iniciar o download
  ...
  function getSelectedClients() {
    ...
    if (ids.length === 0) {
      // se o usuário cancelar, retornamos null (não uma array vazia)
      if (!confirm("Nenhum cliente selecionado. Enviar para todos?")) return null;
  ...
  document.getElementById("enviarWhatsApp").onclick = async () => {
    ...
    for (const id of ids) {
      ...
      // espera curta para não disparar muitas abas de uma vez
